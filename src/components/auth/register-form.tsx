...
export function RegisterForm() {
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (signed?.error) {
        toast.error("تم إنشاء الحساب، فضلًا سجّل الدخول");
        router.push("/login");
      } else {
        toast.success("مرحبًا بك في أفلا يتدبرون!");
        router.push("/");
        router.refresh();
      }
    } finally {
      ...
      <form onSubmit={onSubmit} className="space-y-4">
        <div>
          ...
          <div className="relative">
            <User className="absolute end-3 top-1/2 -translate-y-1/2 h-4 w-4 text-brand-400" />
            <Input id="name" name="name" autoComplete="name" placeholder="اسمك" className="pe-10" />
          </div>
        ...
        <div>
          ...
          <div className="relative">
            ...
            <Input id="password" name="password" type="password" autoComplete="new-password" className="pe-10" />
          </div>
